...
public partial class Form1
{
    ...
    private void ConfigurePersonalization()
    {
        ...
        Shown += (_, _) =>
        {
            try
            {
                ...
                originalTeams = equipes.ToDictionary(
                    ...
                    item => new Equipe(
                        item.Value.Nom,
                        item.Value.Logo,
                        item.Value.MusiqueBut,
                        item.Value.musiqueWarmup));
            ...
            catch (Exception error)
            {
                MessageBox.Show(
                    this,
                    $"Impossible de lire les préférences locales : {error.Message}",
                    "Personnalisation",
        ...
        {
            if (StatusLabel.Text.StartsWith("À jour", StringComparison.Ordinal))
                ApplyPersonalization();
        };
    }
    ...
    private List<Musique> BuildLibraryPlaylist(string category)
    {
        return arenaLibrary!.Tracks(category)
            .Select(track => new Musique(track.Title, track.Path))
            .ToList();
    }
    ...
    private void ApplyPersonalization()
    {
        ...
        foreach (var (identity, original) in originalTeams)
        {
            ...
            if (selection?.Deleted == true) continue;

            var name = string.IsNullOrWhiteSpace(selection?.Name)
                ? original.Nom
                : selection.Name.Trim();

            var goalPath = !string.IsNullOrWhiteSpace(selection?.GoalPath) &&
                           File.Exists(selection.GoalPath)
                ? selection.GoalPath
                : original.MusiqueBut.Fichier;

            var warmupPath = !string.IsNullOrWhiteSpace(selection?.WarmupPath) &&
                             File.Exists(selection.WarmupPath)
                ? selection.WarmupPath
                : original.musiqueWarmup.Fichier;

            AddTeam(updated, name, original.Logo, goalPath, warmupPath);
        }

        foreach (var pair in arenaLibrary.Preferences.Teams
                     ...
                     .OrderBy(pair => pair.Value.Name, StringComparer.CurrentCultureIgnoreCase))
        {
            ...
            if (string.IsNullOrWhiteSpace(selection.Name)) continue;
            ...
            AddTeam(updated, selection.Name.Trim(), string.Empty, goalPath, warmupPath);
        }
        ...
        if (previousLocal is not null && equipes.ContainsKey(previousLocal))
            LocalBox.SelectedItem = previousLocal;

        if (previousVisitor is not null && equipes.ContainsKey(previousVisitor))
            VisiteurBox.SelectedItem = previousVisitor;

        UpdatePlaybackDisplay();
    }

    private void AddTeam(
        Dictionary<string, Equipe> target,
        ...
        string warmupPath)
    {
        ...
        {
            var number = 2;
            while (target.ContainsKey($"{requestedName} ({number})"))
                number++;
            name = $"{requestedName} ({number})";
        }

        var goalTitle = string.IsNullOrWhiteSpace(goalPath)
            ? $"But {name} — aucune musique assignée"
            : $"But {name} — {DisplayName(goalPath)}";

        var warmupTitle = string.IsNullOrWhiteSpace(warmupPath)
            ? $"Échauffement {name} — aucune musique assignée"
            : $"Échauffement {name} — {DisplayName(warmupPath)}";

        target[name] = new Equipe(
            name,
            logo,
            new Musique(goalTitle, goalPath),
            new Musique(warmupTitle, warmupPath));
    }

    private string DisplayName(string path)
    {
        if (arenaLibrary?.Preferences.Titles.TryGetValue(path, out var title) == true &&
