        ...
        private async void SyncButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var progress = new Progress<(int current, int total, string title)>(item =>
                    StatusLabel.Text = $"Téléchargement {item.current}/{item.total} • {item.title}");
                ...
                StatusLabel.Text = $"À jour • {count} pistes téléchargées";
                MessageBox.Show($"{count} pistes sont maintenant disponibles hors ligne.", "Synchronisation terminée", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception exception)
            {
                StatusLabel.Text = "Échec de la synchronisation";
                MessageBox.Show($"Téléchargement impossible.\n\n{exception.Message}", "Erreur", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            finally
            {
                SyncButton.Enabled = true;
                SyncButton.Text = "☁  Télécharger les musiques";
            }
        }
